...
CASE_SPECS = {
    "actino_rjost_rha1": {
        "assembly_accession": "GCA_000014565.1",
        "strict_label": "mtu",
        "soft_neighbors": ["mtu", "sco"],
        "organism_name": "Rhodococcus jostii RHA1",
    },
    "actino_nfar_ifm10152": {
        "nucleotide_accession": "NC_006361.1",
        "strict_label": "mtu",
        "soft_neighbors": ["mtu", "sco"],
        "organism_name": "Nocardia farcinica IFM 10152",
    },
    "actino_sery_nrrl23338": {
        "assembly_accession": "GCA_000062885.1",
        "strict_label": "sco",
        "soft_neighbors": ["sco", "mtu"],
        "organism_name": "Saccharopolyspora erythraea NRRL 23338",
    },
    "firmi_cbei_ncimb8052": {
        "assembly_accession": "GCA_000016965.1",
        "strict_label": "clj",
        "soft_neighbors": ["clj", "bsu"],
        "organism_name": "Clostridium beijerinckii NCIMB 8052",
    },
    "firmi_gkau_hta426": {
        "assembly_accession": "GCA_000009785.1",
        "strict_label": "bsu",
        "soft_neighbors": ["bsu", "clj"],
        "organism_name": "Geobacillus kaustophilus HTA426",
    },
    "firmi_bhal_c125": {
        "assembly_accession": "GCA_000011145.1",
        "strict_label": "bsu",
        "soft_neighbors": ["bsu", "clj"],
        "organism_name": "Halalkalibacterium halodurans C-125",
    },
}
...
def get_default_output_root(repo_root):
    return os.path.join(repo_root, "benchmarks", "query_assets", "phase3_boundary_round2_candidates")
...
def download_via_datasets(spec, destination_path, datasets_executable, case_id):
    ...
    with tempfile.TemporaryDirectory(prefix="phase3-round2-%s-" % case_id) as temp_dir:
...
def download_via_efetch(spec, destination_path):
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "gmsm-phase3-boundary-round2/1.0"},
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download and stage round-2 Phase 3 boundary candidate GenBank assets."
    )
    parser.add_argument(
        "--case-id",
        action="append",
