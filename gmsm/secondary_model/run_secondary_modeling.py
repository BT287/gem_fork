...
def run_secondary_modeling(target_model, io_ns, config_ns, secondary_model_ns):
    ...
    for order, seq_record_BGC_num_list in enumerate(io_ns.seq_record_BGC_num_lists):
        ...
        while region_nr <= total_BGC_single_seq_record:
            if getattr(secondary_model_ns, 'anti_version', None) == 4:
...
def run_sec_met_rxn_generation_anti4(seq_record, order, cluster_nr, target_model, prod_sec_met_dict, nonprod_sec_met_dict, io_ns, config_ns, secondary_model_ns):

    if len(io_ns.seq_record_BGC_num_lists) > 1:
        logging.info("Generating reactions for 'Cluster%s.%s'.." %(order, cluster_nr))
    ...
    get_cluster_product(seq_record, cluster_nr, secondary_model_ns, order, io_ns)
