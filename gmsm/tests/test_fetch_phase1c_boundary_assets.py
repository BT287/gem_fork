...
def load_module():
    script_path = Path(__file__).resolve().parents[2] / "scripts" / "fetch_phase1c_boundary_assets.py"
    spec = importlib.util.spec_from_file_location("fetch_phase1c_boundary_assets", script_path)
...
class TestFetchPhase1cBoundaryAssets:
    ...
    def test_find_single_gbff_prefers_genomic_gbff(self, tmp_path):
        ...
        nested_dir = extract_dir / "ncbi_dataset" / "data" / "GCA_000196335.1"
        nested_dir.mkdir(parents=True)
        (nested_dir / "genomic.gbff").write_text("LOCUS TEST\n", encoding="utf-8")
        (nested_dir / "other.gbff").write_text("LOCUS OTHER\n", encoding="utf-8")

        found = module.find_single_gbff(str(extract_dir), accession="GCA_000196335.1")
    ...
    def test_build_efetch_url_uses_nuccore_gbwithparts(self):
        ...
        url = module.build_efetch_url("NC_018750.1")

        assert "db=nuccore" in url
        assert "NC_018750.1" in url
